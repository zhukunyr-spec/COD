    ...
    with torch.no_grad():
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), save_path + 'Net4_epoch_best.pth')
...
def train(train_loader, model, optimizer, epoch, test_path):
    ...
    if epoch % opt.epoch_save == 0:
        torch.save(model.state_dict(), save_path + str(epoch) + 'ESR4-PVT.pth')
...
if __name__ == '__main__':
    ...
    #torch.load('./COD10K-v3/Test/Image')
